...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        ...
        state = [pred for pred in state if pred not in action.neg]
    ...
    def satisfies(self, state, goal):
        return all(goal_pred in state for goal_pred in goal)
...
def assignments(lvars, lconsts):
    ...
    def makeassign(comb): return dict(zip(lvars, comb))
