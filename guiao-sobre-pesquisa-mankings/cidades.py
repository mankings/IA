...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        for c in [cost for ca, cb, cost in self.connections if (ca, cb) == (c1, c2) or (ca, cb) == (c2, c1)]:
            return c
        return None

    def heuristic(self, city, goal_city):
            x1 = self.coordinates.get(city)[0]
            y1 = self.coordinates.get(city)[1]  
            x2 = self.coordinates.get(goal_city)[0]
            y2 = self.coordinates.get(goal_city)[1]
